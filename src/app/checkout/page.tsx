      ...
      <div className="grid sm:grid-cols-2 gap-4 gap-y-5 flex-wrap">
        ...
        <div className='space-y-3 flex flex-col'>
          <h3>পেমেন্ট মেথড:</h3>
            <div>
            <label className="cursor-pointer">
            <input type="radio" className="peer sr-only" name={"cash"} value={"cash"} checked={payment === "cash"} onChange={() => handleOptionChange("cash")}/>
            <div className={`w-full rounded-md bg-white p-4 text-gray-600 ring-2 transition-all hover:shadow ${"cash" === payment ? 'peer-checked:text-[#fc8934] peer-checked:ring-[#fc8934] peer-checked:ring-offset-2' : ' ring-secondary ring-opacity-10'}`}>
            <div className="flex items-center justify-between gap-1">
              <div className="flex items-centser gap-2">
              <div>
                <CheckCircle color={"cash" === payment ? "#fc8934" : "black"}/>
              </div>
              <p className="text-sm font-semibold uppercase text-gray-500">{"কেশ অন ডেলিভারি"}</p>
              </div>
              <div className="flex items-end justify-between">
              </div>
            </div>
            ...
            <div>
            <label className="cursor-pointer">
            <input disabled={true} type="radio" className="peer sr-only" name={"ssl"} value={"ssl"} checked={payment === "ssl"} onChange={() => handleOptionChange("ssl")}/>
            <div className={`w-full rounded-md bg-white p-4 text-gray-600 ring-2 transition-all hover:shadow ${"ssl" === payment ? 'peer-checked:text-[#fc8934] peer-checked:ring-[#fc8934] peer-checked:ring-offset-2' : ' ring-secondary ring-opacity-10'}`}>
            <div className="flex items-center justify-between gap-1">
              <div className="flex items-centser gap-2">
              <div>
                <CheckCircle color={"ssl" === payment ? "#fc8934" : "black"}/>
              </div>
              <p className="text-sm font-semibold uppercase text-gray-200">{"sslcommerz অনলাইন পেমেন্ট (unavailable)"}</p>
              </div>
              <div className="flex items-end justify-between">
              </div>
            </div>
