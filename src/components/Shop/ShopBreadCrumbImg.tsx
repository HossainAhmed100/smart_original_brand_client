...
const ShopBreadCrumbImg: React.FC<Props> = ({ data, productPerPage, dataType, gender, category })  => {
                      ...
                      <div className="main-content w-full h-full flex flex-col items-center justify-center relative z-[1]">
                          ...
                          <div className="list-tab flex flex-wrap items-center justify-center gap-y-5 gap-8 lg:mt-[70px] mt-12 overflow-hidden">
                              {prCategory.map((item) => (
                                  <div
                                      key={item._id}
                                      className={`tab-item text-button-uppercase cursor-pointer has-line-before line-2px ${dataType === item.path ? 'active' : ''}`}
                                      onClick={() => handleType(item.path)}
                                  >
                                      {item.label}
                                  </div>
                              ))}
                  ...
                  <div className="list-product-block relative">
                      ...
                      >
                          <div className="filter-type">
                            <div className="heading6">Products Category</div>
                            <div className="list-type mt-4">
                              {prCategory.map((item) => (
                                <div
                                  key={item._id}
                                  className={`item flex items-center justify-between cursor-pointer ${dataType === item.path ? 'active' : ''}`}
                                  onClick={() => handleType(item.path)}
                                >
                                  <div className='text-secondary has-line-before hover:text-black capitalize'>{item.label}</div>
                                  <div className='text-secondary2'>
                                    ({data.filter(dataItem => dataItem.category === item.path).length})
                                  </div>
                                </div>
                              ))}
                            </div>
                                  ...
                                  <div className="list-size flex items-center flex-wrap gap-3 gap-y-4 mt-4">
                                      {
                                          ['xs', 's', 'm', 'l', 'xl', '2xl', '3xl'].map((item, index) => (
                                              <div
                                                  key={index}
                                                  className={`size-item text-button w-[44px] h-[44px] uppercase flex items-center justify-center rounded-full border border-line ${size === item ? 'active' : ''}`}
                      ...
                      <div className="list-filtered flex items-center gap-3 mt-4">
                            <div className="total-product uppercase">
                              <span className='text-secondary'>Showing</span>
                              <span className='text-black px-1 font-medium'>{totalProducts}</span>
