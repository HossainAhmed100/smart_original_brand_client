...
const BuyPack = () => {
    ...
    const handleAddToCart = () => {
        if (!cartState.cartArray.find(item => item._id === '123')) {
            addToCart(productData.find(item => item._id === '123') as ProductType)
            updateCart('123', 1, '', '')
        }

        if (!cartState.cartArray.find(item => item._id === '124')) {
            addToCart(productData.find(item => item._id === '124') as ProductType)
            updateCart('124', 1, '', '')
        }

        if (!cartState.cartArray.find(item => item._id === '125')) {
            addToCart(productData.find(item => item._id === '125') as ProductType)
            updateCart('125', 1, '', '')
        }
